...
function setupAutocomplete() {
	...
	autocomplete1.lookupItems = function (value) {
		...
		}, output.specification.archetype);
	};
...
function doGetData() {
	...
	xowl.getCollaborationManifest(function (status, ct, content) {
		if (onOperationEnded(status, content)) {
			for (var i = 0; i != content.outputs.length; i++) {
				if (content.outputs[i].specification.identifier == specId) {
					output = content.outputs[i];
					break;
				}
			}
			if (output !== null && archetypes !== null) {
				renderSpecification();
				doGetArtifacts();
			}
		}
...
function doGetArtifacts() {
	if (output.artifacts.length <= 0)
		return;
	if (!onOperationRequest("Loading ...", output.artifacts.length))
		return;
...
function renderSpecification() {
	document.getElementById("spec-identifier").value = output.specification.identifier;
	document.getElementById("spec-name").value = output.specification.name;
	document.getElementById("spec-archetype").appendChild(renderArchetype(output.specification.archetype));
}
...
function onClickRegister() {
	...
	xowl.registerArtifactForCollaborationOutput(function (status, ct, content) {
		if (onOperationEnded(status, content)) {
